...
const EditPolicy = (props) => {
    ...
    const title = useInput(props.value.editPolicy.title.value, {isEmpty:true, minLength:3});
    ...
    const addORchangeBtn = () => {
        ...
        if (state.AddOrChange == "Add")
            return (<button disabled={disabled} className="btnAddChange" onClick={() => addPolicy()}>Add</button>);
        else if (state.AddOrChange == "Change")
            return (<button disabled={disabled} className="btnAddChange" onClick={() => changePolicy()}>Change</button>);
    ...
    const changePolicy = () => {
        let data = Object.fromEntries(Object.entries(props.value.editPolicy).map(e => [e[0], e[1].value]));
    ...
    useEffect(() => {
        ...
        if (params.id != null) {
            setValue({ AddOrChange: "Change", Loading: props.value.editPolicy.loading });
        }
        else if (params.id == null) {
            setValue({ AddOrChange: "Add", Loading: props.value.editPolicy.loading });
        }
        console.log(state.AddOrChange);
    }, [props.value.editPolicy.loading]);
